# LLMSenpai（Human as LLM） 要件定義書・アーキテクチャ設計 v2.1
...
> **実装状況（#5）**: バックエンド安全基盤を実装済み。①先輩がダッシュボードからコマンド送信すると `Message(commandName, approvalStatus=PENDING)` として保存され、後輩の承認まで実行されない前提で `Session=EXECUTING` にする（`POST /dashboard/sessions/:id/command`）。②危険コマンド検知（`lib/dangerousCommand.ts`：削除・権限変更・外部送信・システム操作・DB破壊 等）で警告理由を返し、先輩ダッシュボードと後輩の承認プロンプトの双方で強調表示する。③後輩側の承認フローAPI（`/kohai/sessions/:id/pending-command` の取得・approve・reject・result）を用意し、承認済みコマンドの実行結果を `role=TOOL` で監査保存する。Discord経由のコマンドも `approvalStatus=PENDING`＋危険検知ログを付与。**後輩の承認プロンプトUIは後輩Web UIチャット本体の実装時に本APIへ接続する。**
...
| **フロントエンド（先輩用ダッシュボード）** | Next.js (React) | **質問キュー画面（#3）＋回答画面（会話履歴・テキスト回答フォーム・担当ロック・コマンド送信＋危険警告＋承認状態表示。§5.2 / §5.4 / §8 / #3・#4・#5）実装済み。キューカードから回答画面へ遷移。履歴/検索ビューは未着手** |
| リアルタイム通信 | SSE（API用）/ WebSocket or SSE（Web UI・ダッシュボード用） | 一部実装済み（キュー画面は短間隔ポーリング。#3） |
| **認証・認可** | **Discord OAuth2（`identify`）＋ Bot によるギルド所属・ロール照会** | **バックエンド＋フロント実装済み（OAuthフロー・JWT Cookieセッション・認可ミドルウェア＋ログイン画面・認証状態ハンドリング・未ログインリダイレクト・ロール別導線。§5.7 / #10）** |
...
2. 先輩ダッシュボード: 質問キュー画面（リアルタイム一覧）【実装済み #3】
3. 先輩ダッシュボード: 回答フォーム & ルーティング【実装済み #3】
4. 先輩ダッシュボード: 担当ロック（重複回答防止）【実装済み #4】
5. コマンド実行の後輩側承認フロー（安全設計・必須）【バックエンド＋先輩UI実装済み #5。後輩承認プロンプトUIは後輩Web UI本体の実装時に接続】
